...
// Padding and border style widths for mouse offsets
    ...
    function handleImage(e) {
        ...
        console.log("Image '%s' loaded", e.target.files[0]);
    }
...
//Initialize a new Box, add it, and invalidate the canvas
    function addRect(motor, baugruppe, einzelteil, x, y, w, h, fill) {
        var rect = new Box2;
        rect.motor = motor;
        rect.baugruppe = baugruppe;
        rect.einzelteil = einzelteil;
        rect.x = x;
        rect.y = y;
        rect.w = w;
        rect.h = h;
        rect.fill = fill || 'rgba(2,165,165,0.7)';
        boxes2.push(rect);
        invalidate();
    }
...
//        addRect('MA', 'C', '01', 260, 70, 60, 65, 'rgba(0,205,0,0.7)');

        // add a green-blue rectangle
//        addRect('MA', 'C', '02', 240, 120, 40, 40, 'rgba(2,165,165,0.7)');

        // add a smaller purple rectangle
//        addRect('MA', 'C', '03', 45, 60, 25, 25, 'rgba(150,150,250,0.7)');
...
//        reloadGruppe();
        ...
        $('#import').click(function() {
            var results = $.parseJSON($('#importArea').val().trim());
            boxes2.length = 0;
        ...
        $('#laden').click(function() {
            $.getJSON("zuordnungen.json", function(data) {
                motoren.length = 0;
                boxes2.length = 0;
                motoren = data;
                reloadMotoren();
...
// unfortunately this can be tricky, we have to worry about padding and borders
    ...
    function reloadGruppe() {
        ...
        for (i = 0; i < boxes2.length; i++) {
            gruppe.options[gruppe.options.length] = new Option(boxes2[i].baugruppe + "-" + boxes2[i].einzelteil, i, false, false);
        }
    }
    ...
    function reloadMotoren() {
        ...
        for (i = 0; i < motoren.length; i++) {
            motor.options[motor.options.length] = new Option(motoren[i].kurz + ", " + motoren[i].name, i, false, false);
        }
    }
